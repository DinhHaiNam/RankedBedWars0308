    ...
    public boolean execute(String[] args, CommandSender s) {
        ...
        if (args.length == 0) {
            ...
            if (foundTeam.isEmpty()) {
                ...
                p.spigot().sendMessage(
                        Misc.msgHoverClick(
                                ss.getPrefix() + "Or if you set the spawn and it wasn't found automatically try using: /bw "
                                        + getSubCommandName() + " <team>",
            ...
            if (ss.getConfig().getYml().get("Team." + args[0]) == null) {
                    ...
                    for (String team : Objects.requireNonNull(
                        ...
                        p.spigot().sendMessage(
                                Misc.msgHoverClick(
                                        ...
                                        ChatColor.WHITE + "Upgrade npc set for "
                ...
                // create hologram
                ss.createUpgradeHologram(p, loc, team);

                String path = "Team." + args[0] + ".Upgrade";

                // existing upgrade locations
                List<String> upgrades =
                        ss.getConfig().getYml().getStringList(path);
                ...
                // add new location
                upgrades.add(serialized);

                // save
                ss.getConfig().getYml().set(path, upgrades);
                ss.getConfig().save();
